...
import { getTaskByNumber, getMembersWithSubmissions, getTaskSubmissions } from "../actions";
...
export default async function TaskDetailPage({ params }: PageProps) {
  ...
  const pendingCount = 0; // No pending since we only show members with submissions
            ...
            <div className="pt-4 border-t">
              ...
              <div className="flex justify-between items-center mt-2">
                ...
                <span className="text-lg font-semibold">
                  {totalMembers > 0 ? Math.round((approvedCount / totalMembers) * 100) : 0}%
                </span>
              </div>
              <div className="flex justify-between items-center mt-2">
                <span className="text-sm text-muted-foreground">Rejection Rate</span>
                <span className="text-lg font-semibold">
                  {totalMembers > 0 ? Math.round((rejectedCount / totalMembers) * 100) : 0}%
                </span>
              </div>
            </div>
          </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {totalMembers === 0 
              ? "No submissions yet" 
              : `${totalMembers} member${totalMembers === 1 ? '' : 's'} submitted this task`
            }
          </CardDescription>
        </CardHeader>
        <CardContent>
